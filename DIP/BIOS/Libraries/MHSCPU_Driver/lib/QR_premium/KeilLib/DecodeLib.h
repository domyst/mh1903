...
typedef struct
{
    bool status;       /*Code128 fnc3状态 true有FNC3标志 false无FNC3标志*/
    uint8_t location;  /*Code128 fnc3位置*/
}Code128FNC3TypeDef;
/*解码数据结构体*/
typedef struct
{
    ...
    Code128FNC3TypeDef c128fnc3; /*记录Code128 FNC3标志和位置*/
...
typedef struct
{
    ...
    uint32_t cfgMatrix2of5;         /*MATRIX 2 OF 5 解码设置*/
    uint32_t cfgIndustrial25;       /*INDUSTRIAL 25 解码设置*/
    uint32_t cfgCode11;             /*CODE11 解码设置       */
    uint32_t cfgCodabar;            /*Codabar 解码设置      */
    uint32_t cfgRSS14;              /*RSS 14 解码设置       */
    uint32_t cfgRSSLimited;         /*RSS Limited 解码设置  */
    uint32_t cfgRSSExpanded;        /*RSS Expanded 解码设置 */
    uint32_t cfgMSIPlessey;         /*MSI Plessey 解码设置  */
    uint32_t cfgStandard2of5;       /*Standard 2of5 解码设置*/
    uint32_t cfgPlessey;            /*Plessey 解码设置      */
    uint32_t cfgChinaPost25;        /*China Post 解码设置   */
    uint32_t cfgCode16K;            /*Code16K 解码设置      */
    /*复合码配置*/
    uint32_t cfgGS1Composite;       /*GS1 Compoiste 解码设置*/ 
    /*二维码配置*/
    uint32_t cfgQRCode;             /*qr code 解码设置      */
    uint32_t cfgPDF417;             /*pdf 417 解码设置      */
    uint32_t cfgDataMatrix;         /*data matrix 解码设置  */
    uint32_t cfgHanXin;             /*汉信 解码设置         */
    uint32_t cfgMicroPDF417;        /*MicroPDF417 解码设置  */
    uint32_t cfgMicroQRCode;        /*MicroQRCode 解码设置  */
    uint32_t cfgMaxiCode;           /*MaxiCode 解码设置     */
    uint32_t cfgAztec;              /*Aztec 解码设置        */
    /*全局解码设置*/
...
typedef enum
{
    ...
    //通用设置 大部分码支持
    DE_EMIT_CHECK = 1,          /*1bit 使能条码输出校验位*/
    DE_ADD_CHECK = 2,           /*2bit 使能条码校验      */
    //code128设置
    DE_CODE128_ENHANCE = 7,     /*7bit 使能code128识读增强，主要提升code128条空比不标准条码*/
    //code39设置
    DE_ASCII         = 3,       /*3bit 使能条码full ASCII模式输出 (code39)*/
    DE_CODE32        = 4,       /*4bit 使能Code39转Code32*/
    DE_CODE32_PREFIX = 5,       /*5bit Code32前缀'A'配置*/
    DE_EMIT_START    = 6,       /*6bit 使能code39起始符输出('*')*/
    DE_EMIT_END      = 7,       /*7bit 使能code39结束符输出('*')*/
    //codabar设置
    DE_CODABAR_MOD16 = 2,       /*2bit Codabar模16校验*/
    DE_CODABAR_MOD10 = 3,       /*3bit Codabar模10校验*/
    DE_CODABAR_ABCD  = 7,       /*7bit Codabar输出起始结束符ABCD*/
    //Code11设置
    DE_CODE11_CHECKC = 2,       /*2bit Code11 C校验（1个校验位）*/
    DE_CODE11_CHECKK = 3,       /*3bit Code11 K校验 （1个校验位）*/
    DE_CODE11_CHECKCK = 4,      /*4bit Code11 C+K校验 （2个校验为都设置优先使用此校验）*/
    //MSI Plessey 设置 （必须开启一个校验，否则不输出）
    DE_MSIPLS_MOD10  = 2,       /*2bit MSI Plessey 模10校验*/
    DE_MSIPLS_MOD1010 = 3,      /*3bit MSI Plessey 双模10校验*/
    //RSS设置
    DE_RSS_AI_NO_BRACKETS = 7,  /*7bit AI项目标识符是否带括号（配置为0带括号）*/
    //EAN UPC
    ...
    DEQR_BLUR          = 2,     /*2bit QRCode精度码算法使能*/
    DEQR_CURVE         = 3,     /*3bit QRCode曲面解码算法使能*/
    DEQR_MODE1         = 5,     /*5bit QRCode模式1使用*/
    DEQR_ECICONFIG     = 7,     /*7bit QRCode输出ECI开关*/

    //DM 状态设置
    DEDM_MULTIPLE      = 1,     /*1bit DataMatrix多码使能*/
    DEDM_ECICONFIG     = 7,     /*7bit DataMatrix输出ECI配置*/
    
    //PDF417设置
    DEPDF417_ECICONFIG  = 7,     /*7bit PDF417输出ECI配置*/
    
    //MicroPDF417 设置
    DEMICPDF417_ECICONFIG = 7,   /*7bit MicroPDF417输出ECI配置*/
    
    //MaxiCode 设置
    DEMAXICODE_ECICONFIG  = 7,   /*7bit MaxiCode 输出ECI配置*/
    
    //汉信 设置
    DEHANXIN_ECICONFIG = 7,      /*7bit 汉信输出ECI配置*/
    
    //Aztec 设置
    DEAZTEC_ECICONFIG = 7,       /*7bit Aztec 输出ECI配置*/
}DeBarCodeStatus_t;
...
typedef enum
{
    ...
    DEBAR_DISABLE     = 24,    /*24bit 全局条码失能*/
    DE_SETUP_C128     = 31,    /*31bit Code128 8位设置码使能*/
...
/**
 * 图像镜像设置
 * TRUE 摄像头出图为镜像图片 FALSE 摄像头输出正常图像
 */
