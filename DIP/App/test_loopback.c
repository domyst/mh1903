...
void loop_back(uint8_t u8Slot)
{
    uint8_t u8Wait = 0;		//When EMV authentication, Set it to 5 sec.
	char *pu8Str = "Class A(5.0V)";
    ...
    printf("Voltage %s\r\n", pu8Str);
	// LCD_DisplayStr(pu8Str, 8, 40, LCD_DISP_GREEN,  LCD_DISP_BLACK );
	...
	//while(button_count == old_count)
    {
        ...
        while ((0 != iso7816_detect(u8Slot)))
        {
            ...
            DBG_PRINT("Wait insert ICC!\r\n");
			// LCD_DisplayStr("Wait insert ICC!             ", 8, 108, LCD_DISP_GREEN,  LCD_DISP_BLACK );
			// LCD_DisplayOneInt(++cnt, 400, 108, LCD_DISP_GREEN, LCD_DISP_BLACK,5);
        ...
        {
            DBG_PRINT("iso7816_init finished!\r\n");
			// LCD_DisplayStr("iso7816_init finished!", 8, 72, LCD_DISP_GREEN,  LCD_DISP_BLACK );
        }
        else
        {
            DBG_PRINT("iso7816_init failed %d!\r\n", s32Stat);
			// LCD_DisplayStr("iso7816_init failed", 8, 72, LCD_DISP_GREEN,  LCD_DISP_BLACK );
        }
...
//            }
            if (rsp.len_out < 4)
            {
				...
				// LCD_DisplayStr("Prepare to send select PSE : ", 8, 108, LCD_DISP_GREEN,  LCD_DISP_BLACK );
				// LCD_DisplayOneInt(++cnt, 400, 108, LCD_DISP_GREEN, LCD_DISP_BLACK,5);
            ...
            else
            {
                ...
                {
                    //Le is present.
                    apdu_req.le = rsp.data_out[rsp.len_out - 1];
                    if (0 == apdu_req.le)
                    {
                        apdu_req.le = 256;
                    }
