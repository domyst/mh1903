...
#define IIC_SCL(n)	((n) ? GPIO_SetBits(GPIOG, GPIO_Pin_13) : GPIO_ResetBits(GPIOB, GPIO_Pin_13))
#define IIC_SDA(n)	((n) ? GPIO_SetBits(GPIOG, GPIO_Pin_14) : GPIO_ResetBits(GPIOB, GPIO_Pin_14))
#define IIC_WP(n)	((n) ? GPIO_SetBits(GPIOG, GPIO_Pin_15) : GPIO_ResetBits(GPIOB, GPIO_Pin_15))
...
static u8 IIC_Wait_Ack(void)
{
	...
	IIC_SCL(1);//IIC_SCL=1;
	delay_us(1);
	// domyst while(GPIO_PinRead(GPIO3, 23U))//while(READ_SDA)
...
static u8 IIC_Read_Byte(unsigned char ack)
{
	...
	for(i=0;i<8;i++ )
	{
		...
		receive<<=1;
		//if(GPIO_PinRead(GPIO3, 23U))receive++;//if(READ_SDA)receive++;
		if(GPIO_ReadInputDataBit(GPIOG, GPIO_Pin_14))receive++;//if(READ_SDA)receive++;
		delay_us(1);
	}
	if (!ack)
		IIC_NAck();
